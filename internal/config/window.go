...
// resolver accepts. Values are base-10 bytes. The default is sourced from
// internal/mux so there is one numeric truth. The 16 MiB max bounds any single
// stream's exposure. Part A has no concurrent-stream cap, so worst-case
// per-process exposure is window × concurrent live streams; that cap (and the
// resulting fixed ceiling) arrives with the Part B lease work — observe process
// memory during the Part A rollout.
const (
	DefaultYamuxStreamWindowBytes int64 = mux.DefaultStreamWindow // 4 MiB
...
// agent startup and thread the value into internal/mux; never read the
