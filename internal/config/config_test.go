...
func TestLoadServer_Valid(t *testing.T) {
	p := writeFile(t, t.TempDir(), "conduitd.yaml", validServerYAML)
	cfg, err := LoadServer(p)
	if err != nil {
		t.Fatalf("LoadServer: %v", err)
	}
	if cfg.BaseDomain != "conduit.example.com" {
		t.Errorf("BaseDomain = %q, want conduit.example.com", cfg.BaseDomain)
	}
...
func TestLoadServer_MissingRequired(t *testing.T) {
	p := writeFile(t, t.TempDir(), "conduitd.yaml", `listen_https: ":8443"`)
...
func TestLoadServer_EnvOverride(t *testing.T) {
	p := writeFile(t, t.TempDir(), "conduitd.yaml", validServerYAML)
	t.Setenv("CONDUIT_BASE_DOMAIN", "override.example.com")
	cfg, err := LoadServer(p)
	if err != nil {
		t.Fatalf("LoadServer: %v", err)
	}
...
func TestLoadClient_MissingFile_DefaultsApplied(t *testing.T) {
	...
	if cfg.DaemonSocket == "" {
		t.Error("DaemonSocket should default to ~/.conduit/daemon.sock")
	}
}
