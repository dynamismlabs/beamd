...
// Entry point for the `@beamd/cli` npm package. The real binary ships in a
// per-platform optional dependency (@beamd/cli-<os>-<cpu>); npm installs only
...
function binaryPath() {
  const pkg = `@beamd/cli-${process.platform}-${process.arch}`;
  let pkgJson;
  try {
